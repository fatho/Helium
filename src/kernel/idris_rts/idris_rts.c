...
VM* init_vm(int stack_size, size_t heap_size, 
    ...
    VAL* valstack = kmalloc(stack_size * sizeof(VAL));
    ...
    vm->inbox = kmalloc(1024*sizeof(VAL));
...
//    return kmalloc(size);
    ...
    if (vm->heap.next + chunk_size < vm->heap.end) {
        ...
        *((size_t*)(vm->heap.next)) = chunk_size;
        ...
        kassert(vm->heap.next <= vm->heap.end);
...
void dumpStack(VM* vm) {
    ...
    for (root = vm->valstack; root < vm->valstack_top; ++root, ++i) {
        kprintf("%d: ", i);
        dumpVal(*root);
        if (*root >= (VAL)(vm->heap.heap) && *root < (VAL)(vm->heap.end)) { kprintf("OK"); }
        kprintf("\n");
    }
    kprintf("RET: ");
    dumpVal(vm->ret);
...
VAL idris_castStrFloat(VM* vm, VAL i) {
    kpanic("Floating point numbers are not supported! Who needs floating point numbers in kernel mode anyway?")
...
//     ThreadData *td = kmalloc(sizeof(ThreadData));
//     td->vm = vm;
//     td->callvm = callvm;
//     td->fn = f;
...
VAL copyTo(VM* vm, VAL x) {
    ...
    case BUFFER:
        ...
        break;
    case BIGINT:
        kpanic("What's the use of big integers in kernel mode, dude?\n")
//        cl = MKBIGMc(vm, x->info.ptr);
    ...
    case BITS64:
        ...
        break;
    default:
        kpanicf("Internal Idris error. Unknown value type %d\n", GETTY(x));
    }
    return cl;
}
